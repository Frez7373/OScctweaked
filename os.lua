...
local function drawTaskbar()
 ...
 term.setCursorPos(math.max(1,W-7),H); write(os.date("%H:%M"))
end
...
local function drawDesktop()
 ...
 for i,a in ipairs(registry) do
  local n=i-1; local col=n%cols; local row=math.floor(n/cols)
  local x=2+col*cellW; local y=4+row*cellH
...
local function launch(meta)
 if not fs.exists(meta.file) then
 ...
 term.clear()
 local ok,err=pcall(function() shell.run(meta.file) end)
 if not ok then
  ...
  term.setTextColor(colors.white); term.setCursorPos(2,4); print(tostring(err)); term.setCursorPos(2,6); print("Touch anywhere to return")
  while true do
...
while true do
 ...
 if e=="term_resize" then
  W,H=term.getSize(); drawDesktop()
 elseif e=="timer" then
  drawDesktop(); os.startTimer(1)
 elseif e=="mouse_click" or e=="monitor_touch" then
  local x,y=p1,p2
  if startOpen then
   local mw=math.min(42,W-2); local mh=math.min(20,H-3); local sx=2; local sy=H-mh-2
   if hit(x,y,sx+1,sy+mh-1,sx+12,sy+mh) then
    startOpen=false; search=""; drawDesktop()
   elseif y>=sy+6 then
  ...
  elseif hit(x,y,1,H-1,9,H) then
   ...
   for i,a in ipairs(registry) do
    ...
    if hit(x,y,bx,by,bx+9,by+2) then launch(a); break end
   end
  end
 elseif e=="char" and startOpen then
  search=search..p1; drawDesktop()
 elseif e=="key" and startOpen and p1==keys.backspace then
  search=search:sub(1,-2); drawDesktop()
 elseif e=="key" and p1==keys.escape then
  startOpen=false; search=""; drawDesktop()
 elseif e=="peripheral" or e=="peripheral_detach" then
  say("Device connection changed",3); drawDesktop()
 end
end
